...
(function (window, document) {
  ...
  var marqueeViewportWidth = null;

  function getMarqueeViewportWidth() {
    return document.documentElement.clientWidth || window.innerWidth || 0;
  }

  function prepareMarqueeWrapper(wrapper, options) {
    if (!wrapper) return;

    wrapper.classList.add('cp-tpl-marquee');
    wrapper.classList.toggle('cp-tpl-marquee--reverse', options.reverse === true);
  }

  function buildMarquee(wrapper, options) {
    var track = wrapper.querySelector(':scope > div');

    if (!track) return;

    prepareMarqueeWrapper(wrapper, options);
    ...
    var viewportWidth = getMarqueeViewportWidth();

    while (track.scrollWidth < viewportWidth * options.minWidthFactor) {
      originalItems.forEach(function (item) {
  ...
  function rebuildAllMarquees() {
    marqueeItems.forEach(function (item) {
      document.querySelectorAll(item.selector).forEach(function (wrapper) {
        buildMarquee(wrapper, item);
      });
    });
  }

  function bindMarqueeResize() {
    if (marqueeResizeBound) return;

    marqueeResizeBound = true;
    marqueeViewportWidth = getMarqueeViewportWidth();

    window.addEventListener('resize', function () {
      var nextViewportWidth = getMarqueeViewportWidth();

      // iOS Safari fires resize while its browser chrome expands/collapses
      // during a vertical scroll. Ignore it unless the layout width changed.
      if (nextViewportWidth === marqueeViewportWidth) {
        return;
      }

      marqueeViewportWidth = nextViewportWidth;
  ...
  cp.marquee = function (config) {
    ...
    ensureStyle('cp_tpl_marquee_style', [
      ...
      '  flex: 0 0 auto;',
      '}',
      '.cp-tpl-marquee.is-ready > div {',
      '  animation: marqueeInfinite var(--marquee-duration) linear infinite;',
      '}',
      '.cp-tpl-marquee.cp-tpl-marquee--reverse.is-ready > div {',
  ...
  cp.cjm = cp.cjm || {};
    ...
    cp.cjm.products = [{
        ...
        },
        {
            ...
            kitTariffUuid: 'b01d1028-b73c-45e8-87a7-bc126302c5ef'
        },
        {
            brand: 'skysmart',
            id: 'kid_skysmart_homeschooling_5_grade',
            label: '5 класс',
            selectValues: ['Домашний лицей 5 класс'],
            productKitCode: 'skysmart_homeschooling_5_grade',
            kitTariffUuid: '55c18d55-b9fc-4a0c-a541-d3a7299cfe75'
        },
        {
            brand: 'skysmart',
            id: 'kid_skysmart_homeschooling_6_grade',
            label: '6 класс',
            selectValues: ['Домашний лицей 6 класс'],
            productKitCode: 'skysmart_homeschooling_6_grade',
            kitTariffUuid: 'acfeaa6f-91f6-4f9e-94d3-8a8434260bc9'
        },
        {
            brand: 'skysmart',
            id: 'kid_skysmart_homeschooling_7_grade',
            label: '7 класс',
            selectValues: ['Домашний лицей 7 класс'],
            productKitCode: 'skysmart_homeschooling_7_grade',
            kitTariffUuid: '0c2343c8-5fa9-42de-bc2f-7597dea41ea6'
        },
        {
            brand: 'skysmart',
            id: 'kid_skysmart_homeschooling_8_grade',
            label: '8 класс',
            selectValues: ['Домашний лицей 8 класс'],
            productKitCode: 'skysmart_homeschooling_8_grade',
            kitTariffUuid: 'b01d1028-b73c-45e8-87a7-bc126302c5ef'
        }, 
        {
            ...
            label: '9 класс',
            selectValues: ['Домашний лицей 9 класс'],
            productKitCode: 'skysmart_homeschooling_9_grade',
            kitTariffUuid: '0952b74d-bd1c-4efb-9e62-0fba4cedefba'
        },
        {
            brand: 'skysmart',
            id: 'kid_skysmart_homeschooling_10_grade',
            label: '10 класс',
            selectValues: ['Домашний лицей 10 класс'],
            productKitCode: 'skysmart_homeschooling_10_grade',
            kitTariffUuid: 'fc1286a3-59f4-4941-980f-74bed3e7a280'
        },
        {
            brand: 'skysmart',
